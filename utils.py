...
class Batchify:
    def __init__(self, data, tokenizer, bos, eos, batch_size=128, shuffle=False):
        ...
        self.index =[[0 for col in range(11111)] for row in range(11111)]
        # print(data)
        j = 0
        for x in data:
            u.append(x['user'])
    ...
    def search(self,user_id,item_id):
        for a,b,c,d  in zip(self.u,self.i,self.r,self.t):
           if (str(a) == user_id) and (str(b) == item_id):
               mid1 = []
               mid2 = []
               mid3 = []
               mid4 = []
               ...
               mid4.append(d)
               # mid5.append(e)
               user = torch.tensor(mid1, dtype=torch.int64).contiguous()
               item = torch.tensor(mid2, dtype=torch.int64).contiguous()
               rating = torch.tensor(mid3, dtype=torch.float).contiguous()
               seq = self.seq[[self.index[int(user_id)][int(item_id)]]]  # (batch_size, seq_len)
               mask = self.mask[[self.index[int(user_id)][int(item_id)]]]
               # feature= torch.tensor(mid5, dtype=torch.int64).contiguous()
               # print(self.u)
               return user, item, rating, seq,mask
